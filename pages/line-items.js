...
function getSubmittedBy(row) {
  ...
  const ccName = fromId("55"); // CC form name
  if (ccName) return String(ccName).trim();
  const invPurchaser = fromId("33"); // Invoice purchaser
...
/** Display date rules
 * CC: Return date (28.prettyFormat) || sub.created_at
 * Invoice: Invoice date (31.prettyFormat) || sub.created_at
 */
...
function InvoiceStructuredView({ row }) {
  const fields = [
    ...
    ["Program", row.program || "—"],
    ["Billed To", row.billedTo || row.program || "—"],
...
export default function LineItems() {
  ...
  // Moved here so it has access to `items`
      ...
      <Dialog open={!!modalRow} onClose={() => setModalRow(null)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ pr: 7 }}>
          ...
          {!!modalRow?.customer && isYHDPFlex(modalRow) && (() => {
            const k = modalRow.customer.trim().toLowerCase();
